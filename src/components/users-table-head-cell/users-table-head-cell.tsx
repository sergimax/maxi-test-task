...
export const UsersTableHeadCell = ({
  label, onSorting, sortingParams, onInputChange,
}: UsersTableHeadCellProps) => {
  const classNames = onSorting ? styles.sortable : '';

  const sortingTrend = sortingParams?.key === label ? sortingParams.direction === SORTING_DIRECTION.ASC ? ' ▲' : ' ▼' : '';
  ...
  return (<th className={classNames}>
      <div className={styles['head-cell']}>
        ...
        {onInputChange && (<TextField size={"small"}
